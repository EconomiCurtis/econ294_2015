...
Use R to answer the following answers. 

Put solutions in an `.R` script file. Name the file by concatinating your first and last name, plus "Assignment1". (e.g. my file would be called `CurtisKephartAssignment1.R`)
...
Comment your `.R` script 

Due date: Jan. 15 2016

0. Use a `print` call to report your first and last name, and student ID number. 
...
`https://github.com/EconomiCurtis/econ294_2015/raw/master/data/NHIS_2007_TSV.txt`
  ...
  + From .RData. Note that `.RData` files come with names already assigned to their data structures. What is the name assigned to this RData file?
  
  `https://github.com/EconomiCurtis/econ294_2015/raw/master/data/NHIS_2007_RData.RData`


2. Download each file above to your hard drive, how big (in `KB`) is each file? Which is the smallest? Besides the .dta file, what accounts for their variability? 
...
5. Create the following vector, `v <- c(1,2,3,4,5,6,7,4,NULL,NA)`
